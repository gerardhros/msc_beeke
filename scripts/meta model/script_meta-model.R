...
m1.cov.mean <- dcast(data = m1.covar, 
                     formula = ES + SE + n + `moderator 1`  ~ ind_code, 
                     value.var = 'wm.mean', 
                     fun.aggregate = mean)
setnames(m1.cov.mean, colnames(m1.cov.mean)[-c(1:7)], paste0('wm_', colnames(m1.cov.mean)[-c(1:7)]))
m1.cov.sd <- dcast(data = m1.covar, 
                     formula = ES + SE + n + `moderator 1`  ~ ind_code, 
                     value.var = 'wm.sd', 
                     fun.aggregate = mean)
setnames(m1.cov.sd, colnames(m1.cov.sd)[-c(1:7)], paste0('wm_sd_', colnames(m1.cov.sd)[-c(1:7)]))

#merge m1.main.mean and m1.main.sd
m1.cov.merged <- merge(m1.cov.mean, m1.cov.sd, 
                       by = c("moderator 1", "ES", "SE", "n"), 
                       all.x = TRUE)
...
m1.cov.merged[`moderator 1` == "soil pH", `moderator 1` := "soil ph"]
m1.cov.merged[`moderator 1` == "bc pH", `moderator 1` := "bc ph"]
