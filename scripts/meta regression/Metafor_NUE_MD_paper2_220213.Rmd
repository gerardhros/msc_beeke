...
```{r load data,warning=FALSE,message=FALSE}
  ...
  # read data
  d1 <- readxl::read_xlsx('E:/phD/Papers/paper2/You_et_al_2022/public/20220307_Database impacts measures on NUE.xlsx',sheet = 1)
  d1 <- as.data.table(d1)
...
```{r forest plot per treatment_ROM,warning=FALSE}
...
# do a first main factor analysis for log response ratio for NUE
  ...
  # update the missing values for n_dose and p_dose (as example)
...
# i select only one example
  ...
  Fr_nue_0 <- rma.mv(yi,vi, data = d02,random= list(~ 1|studyid), method="REML",sparse = TRUE)
...
check croptypemarize??? and also vegetable vs. vegetables (I have changed marize to maize and vegetables to vegetable.)
...
# here just "g_n_dose"
  r_nue_03 <- rma.mv(yi,vi, 
                  mods = ~g_n_dose - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_03,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_03)
  ...
  # here just "g_p_dose"
  r_nue_04 <- rma.mv(yi,vi, 
                  mods = ~g_p_dose - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_04,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_04)
  ...
  # here just "g_k_dose"
  r_nue_05 <- rma.mv(yi,vi, 
                  mods = ~g_k_dose - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_05,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_05) 
  
  # here just "evaporation"
  ...
  # here just "g_evaporation"
  r_nue_06 <- rma.mv(yi,vi, 
                  mods = ~g_evaporation - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_06,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_06) 
  ...
  # here just "g_elevation"
  r_nue_09 <- rma.mv(yi,vi, 
                  mods = ~g_elevation - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_09,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_09) 
  
  # here just "bulk_density"
  ...
  # here just "g_bulk_density"
  r_nue_010 <- rma.mv(yi,vi, 
                  mods = ~g_bulk_density - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_010,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_010)  
  ...
  # here just "total_nitrogen"
  r_nue_13 <- rma.mv(yi,vi, 
                  mods = ~total_nitrogen - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)
  ...
  # here just "g_total_nitrogen"
  r_nue_013 <- rma.mv(yi,vi, 
                  mods = ~g_total_nitrogen - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_013,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_013)  
  ...
  # here just "g_soc"
  r_nue_014 <- rma.mv(yi,vi, 
                  mods = ~g_soc - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_014,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_014)
  ...
  # here just "g_ph"
  r_nue_015 <- rma.mv(yi,vi, 
                  mods = ~g_ph - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)

  # show stats and improvements
  out = estats(model_new = r_nue_015,model_base = r_nue_0)
  print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
  summary(r_nue_015) 
  
  # 2. here with 2 variables
  
  # here with 2 variables "tillage, g_crop_type"
  ...
  # here with 2 variables "tillage, n_dose"
  ...
  # here with 2 variables "g_crop_type, n_dose"
  r_nue_18 <- rma.mv(yi,vi, 
                  mods = ~g_crop_type + n_dose - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)
  ...
  # here with 3 variables "tillage, g_crop_type, n_dose"
  r_nue_19 <- rma.mv(yi,vi, 
                  mods = ~tillage + g_crop_type + n_dose - 1, 
                  data = d02,
                  random = list(~ 1|studyid), method="REML",sparse = TRUE)
...
```{r model predictions, warning=FALSE,message=FALSE, eval=FALSE}
  ...
  m2 = predict(r_nue_19,newmods=as.matrix(dt.pred),transf=exp,addx=T) 
